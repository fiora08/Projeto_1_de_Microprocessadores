...
// todos os delays usados aqui é para o sistema possa identificar as mudanças fisicamente. 
// como mudança na tensão.
// LCD_DADOS_PORT é o portc
// LCD_CONTR_PORT É o portb
// aqui é o controle do enable que valida o envio de informação pro lcd
...
void lcd_comando(unsigned char comando) {
	LCD_CONTR_PORT &= ~(1 << RS);    // RS em 0 (Instrução)
...
void lcd_inicializar(void) {
	...
	_delay_ms(100); // Aguarda o LCD estabilizar a tensão

	// Sequência de inicialização forçada para modo 4 bits
...
void lcd_escrever_string(char* texto) {
	while (*texto) { // enquanto não chega o final '\0' da string 
...
void lcd_posicionar(unsigned char linha, unsigned char coluna) {
	unsigned char posicao = (linha == 0) ? (0x80 + coluna) : (0xC0 + coluna); // // 0x80 é o endereço da linha 1, 0xC0 o da linha 2
	lcd_comando(posicao); // move o cursor para a posição calculada
}

void lcd_limpar(void) {
	lcd_comando(0x01);  // // Comando 0x01: Clear Display
	_delay_ms(5); // Delay maior para a limpeza da memória interna
}