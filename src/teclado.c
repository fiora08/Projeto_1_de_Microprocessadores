#include <avr/io.h>
#include "teclado.h"
#include "timers.h" // Necess�rio para a flag_1ms



// Vari�veis internas da biblioteca
static unsigned char tecla_estavel = 0;
static unsigned char tecla_atual = 0;
static unsigned char contador_estabilidade = 0;

// Mapa de teclas conforme o teclado alfanum�rico de 16 teclas 
...
// Realiza a varredura f�sica nos pinos do PORTA
unsigned char teclado_ler_bruto(void) {
	unsigned char linha, coluna_lida;
	for (linha = 0; linha < 4; linha++) {
		// O deslocamento 0x10 come�a em PA4
		PORTA = ~(0x10 << linha) | 0x0F;
		// Pequena espera para estabiliza��o el�trica dos pinos
		for (volatile unsigned char i = 0; i < 10; i++) asm("nop"); // "asm(nop)" � um atraso de nanosegundos para dar um respiro a fun��o.
		// L� o estado das colunas nos pinos PA0-PA3
		coluna_lida = PINA & 0x0F;
		// Verifica se algum bit de coluna � 0 que � tecla pressionada
		if (coluna_lida != 0x0F) {
...
void teclado_inicializar(void) {
	// PA4-PA7 como sa�das e PA0-PA3 como entradas 
	DDRA = 0xF0;
...
void teclado_atualizar(void) {
	...
	if (flag_1ms) {
		...
		// aqui tem a verifica��o de ruido
		...
		// Se a tecla permanecer est�vel pelo tempo definido 20ms
		if (contador_estabilidade >= DEBOUNCE_MAXIMO) {
...
// fun��o para ler a tecla e retornar a tecla pressionada
unsigned char teclado_obter_tecla(void) {
	...
	ultima_tecla_enviada = tecla_estavel; // trava o envio at� o usuario parar de pressionar a tecla
	return retorno;
}
